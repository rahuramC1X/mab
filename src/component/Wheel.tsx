...
const MyRouletteComponent: React.FC = () => {
  ...
  const generateDivisionsWithProbabilities = (probabilities: number[]): Division[] => {
    ...
    const colors = ['#FF5733', '#FFC300', '#DAF7A6', '#C70039', '#581845', '#900C3F', '#FF5733', '#FFC300', '#DAF7A6', '#C70039'];
  ...
  return (
    <div className="roulette-container">
      ...
      {wheels.map((wheel, index) => (
        <div key={index} className="wheel-container">
          <svg className={`wheel ${wheel.mustSpin ? 'spin' : ''}`} viewBox="0 0 300 300">
            ...
            {wheel.divisions.map((division, idx) => (
              <g key={idx}>
                ...
                <text x={150 + 100 * Math.cos(((idx + 0.5) / 10) * Math.PI * 2)} y={150 + 100 * Math.sin(((idx + 0.5) / 10) * Math.PI * 2)} textAnchor="middle" dominantBaseline="middle" fill="white">{division.number}</text>
              </g>
            ))}
            {wheel.prizeNumber !== null && (
              ...
              x1="150"
              y1="150"
              x2={150 + 55 * Math.cos(((wheel.prizeNumber + 0.5) / 10) * Math.PI * 2)} // Adjusted length to half (from 110 to 55)
              y2={150 + 55 * Math.sin(((wheel.prizeNumber + 0.5) / 10) * Math.PI * 2)} // Adjusted length to half (from 110 to 55)
              stroke="black"
              strokeWidth="6" // Adjusted strokeWidth for better proportion
              strokeLinecap="round"
            />
            )}
          </svg>
          <div className="result">{wheel.prizeNumber != null && wheel.divisions[wheel.prizeNumber].number}</div>
          <button onClick={() => {totalSpinCount += 1 
            handleSpinClick(index)}} disabled={wheel.mustSpin}>SPIN</button>
        </div>
      ))}
